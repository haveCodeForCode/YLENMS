    ...
    $.extend({
    	...
    	table: {
            ...
            initEvent: function() {
            	...
            	// 单击tooltip事件
            	$(optionsIds).on("click", '.tooltip-show', function() {            		var target = $(this).data('target');
            		var input = $(this).prev();
        ...
        operate: {
			...
			reboot : function(id) {
            	table.set();
				$.modal.confirm("确定重启该" + table.options.modalName+ "吗？",
					function() {
						var url = $.common.isEmpty(id) ? table.options.rebootUrl: table.options.rebootUrl.replace("{id}", id);
						var data = {
							"ids" : id
						};
						$.operate.submit(url, "post","json", data);
			...
			shutdown : function(id) {
            	table.set();
				$.modal.confirm("确定关闭该" + table.options.modalName+ "吗？",
					function() {
						var url = $.common.isEmpty(id) ? table.options.shutdownUrl: table.options.shutdownUrl.replace("{id}", id);
						var data = {
							"ids" : id
						};
						$.operate.submit(url, "post","json", data);
			...
			config : function(id) {
				...
				var url = "/404.html";
				if ($.common.isNotEmpty(id)) {
					url = table.options.configUrl.replace("{id}", id);
				} else {
					var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns(): $.table.selectColumns(table.options.uniqueId);
					if (id.length == 0) {
						$.modal.alertWarning("请至少选择一条记录");
				...
				$.modal.openNoBtn("配置" + table.options.modalName,url);
			},
			preview : function(id) {
            	table.set();
				var url = "/404.html";
				...
				$.modal.openNoBtn("预览" + table.options.modalName,	url);
			},
			// 成功回调执行事件（父窗体静默更新）
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.YLENMS_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
