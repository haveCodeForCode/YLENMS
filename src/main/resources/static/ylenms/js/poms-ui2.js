/**
 * 通用js方法封装处理 Copyright (c) 2018 ylenms
 */
(function($) {
	$
			.extend({
								...
								.each(
										...
										function(index, dict) {
											if (dict.dictValue == ('' + value)) {
												var listClass = $.common
														.equals("default",
																dict.listClass)
														|| $.common
																.isEmpty(dict.listClass) ? ""
														: "badge badge-"
																+ dict.listClass;
												actions
														.push("<span class='badge badge-"
																+ dict.listClass
																+ "'>"
																+ dict.dictLabel
																+ "</span>");
				...
				modal : {
					...
					parentTab : function(title, url) {
						var dataId = window.frameElement
				...
				operate : {
					...
					reboot : function(id) {
						$.modal
								.confirm(
										"确定重启该条" + $.table._option.modalName
												+ "吗？",
										function() {
											var url = $.common.isEmpty(id) ? $.table._option.rebootUrl
													: $.table._option.rebootUrl
															.replace("{id}", id);
											var data = {
												"ids" : id
											};
					...
					shutdown : function(id) {
						$.modal
								.confirm(
										"确定关闭该条" + $.table._option.modalName
												+ "吗？",
										function() {
											var url = $.common.isEmpty(id) ? $.table._option.shutdownUrl
													: $.table._option.shutdownUrl
															.replace("{id}", id);
											var data = {
												"ids" : id
											};
					...
					config : function(id) {
						var url = "/404.html";
						if ($.common.isNotEmpty(id)) {
							url = $.table._option.configUrl.replace("{id}", id);
						} else {
							var id = $.common.isEmpty($.table._option.uniqueId) ? $.table
									.selectFirstColumns()
									: $.table
						...
						$.modal
								.openNoBtn("配置" + $.table._option.modalName,
										url);
					},
					preview : function(id) {
						var url = "/404.html";
						if ($.common.isNotEmpty(id)) {
							url = $.table._option.previewUrl
									.replace("{id}", id);
						} else {
							var id = $.common.isEmpty($.table._option.uniqueId) ? $.table
									.selectFirstColumns()
						...
						$.modal
								.openNoBtn("预览" + $.table._option.modalName,
										url);
					},
					// 添加信息 全屏
					addFull : function(id) {
					...
					saveunclose : function(url, data) {
						var config = {
							...
							success : function(result) {
								$.operate.successCallbackUnclose(result);
							}
						};
						$.ajax(config)
					},
					...
					// 成功回调执行事件（父窗体静默更新）
					successCallbackUnclose : function(result) {
						if (result.code == web_status.SUCCESS) {
							var parent = window.parent;
							if (parent.$.table._option.type == table_type.bootstrapTable) {
								// $.modal.close();
					...
					successTabCallback : function(result) {
						if (result.code == web_status.SUCCESS) {
							...
							var $contentWindow = $('.YLENMS_iframe[data-id="'
